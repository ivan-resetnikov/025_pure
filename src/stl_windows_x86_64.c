...
P_Error P_IOStream_Seek(P_IOStream* p_stream, long offset, P_IOStream_SeekMode mode)
{
    ...
    int origin;
    switch (mode) {
        case IO_SEEK_MODE_SET: origin = SEEK_SET; break;
        case IO_SEEK_MODE_CUR: origin = SEEK_CUR; break;
        case IO_SEEK_MODE_END: origin = SEEK_END; break;
...
P_Error P_IOStream_Tell(P_IOStream* p_stream, long* out_position)
{
    P_assert(p_stream != P_NULL, "p_stream is null!");
    P_assert(out_position != P_NULL, "out_position is null!");

    long pos = ftell(p_stream->stream);
    if (pos < 0) {
